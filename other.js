...
//created the os
const osName = os.arch();

const infoTextFile = path.join(__dirname, 'info.txt');
const infoTextContent = `This is being run on a ${osName} computer!`;

try {
  fs.writeFileSync(infoTextFile, infoTextContent, 'utf-8');
  console.log('info.txt created in the client folder.');

  // Read the 'info.txt' file
  const content = fs.readFileSync(infoTextFile, 'utf-8');
  console.log('File content:', content);
} catch (error) {
  console.error('Error creating info.txt file:', error.message);
}
